// src/pages/exam.jsx  —  Trang làm bài thi
...
export default function ExamPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session: s } }) => {
      ...
      // Kiểm tra có session đang thi không
  ...
  if (phase === 'start') return (
    <div className={styles.center}>
      <div className={styles.startCard}>
        ...
        <button className={styles.logoutLink} onClick={async () => {
  ...
  if (phase === 'result') return (
    <div className={styles.center}>
      <div className={styles.resultCard}>
        ...
        <button className={styles.startBtn} style={{marginTop:24}} onClick={() => {
          setPhase('start'); setAnswers({}); setQuestions([]); setSession(null); setResult(null);
        }}>
          Làm bài lại
        </button>
      </div>
  ...
  // ── RENDER: Exam ────────────────────────────────────────────
  if (!q) return null;
  const isLow = timeLeft < 300; // cảnh báo đỏ khi < 5 phút
            ...
            {questions.map((question, idx) => (
              <button
                ...
                className={`${styles.qMapBtn}
                  ...
                  ${answers[question.id] ? styles.qMapAnswered : ''}
                `}
