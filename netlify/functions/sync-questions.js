...
exports.handler = async (event) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      questions.push({
        sheet_row_id: `row_${i}`,   // FIX: stable ID, no Date.now()
    ...
    if (questions.length === 0) {
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ message: 'Không có câu hỏi hợp lệ', synced: 0 }),
      };
    }
    ...
    // FIX: 先删除所有旧数据，再插入新数据
    console.log('[sync-questions] Đang xóa câu hỏi cũ...');
    ...
    console.log('[sync-questions] Đã xóa câu hỏi cũ ✅');
