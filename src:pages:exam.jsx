  ...
  if (phase === 'start') return (
    <div className={styles.center}>
      <div className={styles.startCard}>
        ...
        <button className={styles.logoutLink} onClick={async () => { await supabase.auth.signOut(); router.replace('/'); }}>Đăng xuất</button>
      </div>
    </div>
  );
  ...
  if (phase === 'result') return (
    <div className={styles.center}>
      <div className={styles.resultCard}>
        <div className={`${styles.scoreCircle} ${result.score >= 70 ? styles.pass : styles.fail}`}>
          <span className={styles.scoreNum}>{result.score}</span>
          <span className={styles.scorePct}>điểm</span>
        </div>
        <h2 className={styles.resultTitle}>
          {result.score >= 70 ? '🎉 Chúc mừng bạn đã vượt qua!' : '📚 Bạn cần ôn tập thêm'}
        </h2>
        <p className={styles.resultStat}>Đúng <strong>{result.correct}</strong> / {result.total} câu</p>
        <button className={styles.startBtn} style={{marginTop:24}} onClick={() => { setPhase('start'); setAnswers({}); setQuestions([]); setSession(null); setResult(null); }}>
          Làm bài lại
        </button>
      </div>
    </div>
    ...
    <div className={styles.examPage}>
      <header className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <span className={styles.progress}>{answeredCount}/{questions.length} đã trả lời</span>
          {saving && <span className={styles.saving}>Đang lưu...</span>}
        </div>
        <div className={`${styles.timer} ${isLow ? styles.timerLow : ''}`}>⏱ {formatTime(timeLeft)}</div>
      ...
      <div className={styles.examBody}>
        <main className={styles.questionPanel}>
          ...
          <div className={styles.options}>
            {['a','b','c','d'].map(opt => (
              <button key={opt} className={`${styles.option} ${answers[q.id] === opt ? styles.selected : ''}`} onClick={() => handleAnswer(q.id, opt)}>
          ...
          <div className={styles.nav}>
            <button className={styles.navBtn} onClick={() => setCurrent(c => Math.max(0, c - 1))} disabled={current === 0}>← Câu trước</button>
            <button className={`${styles.navBtn} ${styles.navNext}`} onClick={() => setCurrent(c => Math.min(questions.length - 1, c + 1))} disabled={current === questions.length - 1}>Câu tiếp →</button>
          </div>
        </main>
        <aside className={styles.sidebar}>
          ...
          <div className={styles.qMap}>
            {questions.map((question, idx) => (
              <button key={question.id} className={`${styles.qMapBtn} ${idx === current ? styles.qMapCurrent : ''} ${answers[question.id] ? styles.qMapAnswered : ''}`} onClick={() => setCurrent(idx)}>
                {idx + 1}
              </button>
            ))}
          </div>
